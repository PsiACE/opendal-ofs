...
impl ContentReuseLookup {
    ...
    pub fn file(&self, content: ContentRef) -> Result<Option<FileExtentMap>, Error> {
    ...
    #[allow(dead_code)]
    pub(crate) fn empty(workspace: &WorkContext) -> Result<Self, Error> {
        let files = workspace.writer("empty-known-files")?.finish()?;
        let extents = workspace.writer("empty-known-extents")?.finish()?;
        Self::build(workspace, &files, &extents)
    }
}

pub(crate) async fn build_known_content<A: VolumeAccess>(
    access: &A,
    operator: &Operator,
    workspace: &WorkContext,
...
async fn read_extent_run<A: VolumeAccess>(
    access: &A,
    operator: &Operator,
    workspace: &WorkContext,
    run: ExtentRunRef,
    ...
    let mut source = run.extents(operator).await?;
