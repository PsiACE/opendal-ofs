...
use super::reuse::ContentReuseLookup;
use super::segment::DataSegmentWriter;

pub(crate) async fn write_file<A: VolumeAccess>(
    access: &A,
    operator: &Operator,
...
pub(crate) async fn write_patch<A: VolumeAccess>(
    access: &A,
    operator: &Operator,
    multipart_part_bytes: NonZeroUsize,
    placement: &mut DataSegmentWriter<'_>,
    ...
    let data = if replaces_file {
        ...
        previous
            .1
            .add_run(operator, placement.gc_epoch(), multipart_part_bytes, patch)
            .await?
    };
