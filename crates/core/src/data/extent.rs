...
impl FileExtentMap {
    ...
    pub(crate) async fn extents(
        ...
        for reference in self.runs() {
            ...
            let mut run = reference.extents(operator).await?;
    ...
    pub(crate) async fn add_run(
        ...
        for level in 0..u64::BITS as usize {
            if self.patch_levels.len() == level {
                self.patch_levels.push(Some(
                    materialize_runs(operator, gc_epoch, multipart_part_bytes, runs, |_| Ok(()))
                        .await?,
                ));
                return Ok(self);
            }
            match self.patch_levels[level].take() {
                None => {
                    self.patch_levels[level] = Some(
                        materialize_runs(
                            operator,
                            gc_epoch,
                    ...
                    while self.patch_levels.last().is_some_and(Option::is_none) {
                        self.patch_levels.pop();
                    }
                    return Ok(self);
                }
        ...
        runs.push(self.base_run.take().ok_or_else(|| {
            Error::corrupt(
        ...
        self.base_run = Some(
            materialize_runs(operator, gc_epoch, multipart_part_bytes, runs, |_| Ok(())).await?,
        );
        self.patch_levels.clear();
        Ok(self)
    }
    ...
    pub(crate) async fn compact(
        ...
        if self.base_run.is_none() {
            return Ok(Self::empty());
        }
        if self.patch_levels.is_empty() {
            let mut extents = self.extents(operator, None).await?;
            while let Some(mapping) = extents.next().await? {
                visit(&mapping)?;
            }
            return Ok(self.clone());
        }
        let runs = self.runs().cloned().collect();
        Ok(Self::from_base(
...
async fn materialize_runs(
    ...
    let mut extents = overlay.extents(operator, None).await?;
    while let Some(mapping) = extents.next().await? {
        visit(&mapping)?;
...
impl ExtentRunRef {
    /// Open this run's ordered extent records without applying other levels.
    pub(crate) async fn extents(&self, operator: &Operator) -> Result<FileExtentRunReader, Error> {
        let tail = match self.continuation {
            Some(tail) => Some(RecordStreamReader::open(operator, tail).await?),
            None => None,
        };
        Ok(FileExtentRunReader {
            first: Some(self.inline_extent.clone()),
