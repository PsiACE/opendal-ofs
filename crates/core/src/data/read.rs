...
use crate::format::{ExtentRef, FileExtentMap, FileRange, ObjectClass, ObjectLocator};
...
pub(crate) async fn read_file<A: VolumeAccess>(
    access: &A,
    operator: &Operator,
    read_gap_bytes: usize,
    ...
    let mut mappings = reference.extents(operator, Some(selection)).await?;
    let (mut existing, mut reusable_source) = match reusable {
        Some(reusable) => (
            Some(
                reusable
                    .reference
                    .extents(operator, Some(selection))
                    .await?,
            ),
