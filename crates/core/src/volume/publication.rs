...
use crate::format::{
    ...
    ObjectClass, OperationReceipt, OperationReceiptSegment, take_merge_tail,
};
use crate::storage::{ImmutableWriter, RecordStreamReader, RecordStreamSizer, RecordStreamWriter};
use crate::work::{WorkContext, sort};

use super::namespace::{self, Namespace};
use super::open::{ManagedObservation, ManagedVolume};
            ...
            if current_content != Some(file.content) {
                ...
                let data = data
                    .compact(
                        ...
                        |mapping| {
                            self.access().validate_extent(&mapping.extent)?;
...
async fn write_operation_receipts<A: VolumeAccess>(
...
pub(super) async fn write_commit<A: VolumeAccess>(
    volume: &ManagedVolume<A>,
    gc_epoch: GcEpoch,
...
pub(super) async fn read_commit<A: VolumeAccess>(
    volume: &ManagedVolume<A>,
    reference: NamespaceRevision,
...
async fn operation_in_commit<A: VolumeAccess>(
    volume: &ManagedVolume<A>,
    operation: OperationId,
    expected_cursor: ChangeCursor,
    commit: &NamespaceCommit,
...
async fn read_operation_receipt<A: VolumeAccess>(
    volume: &ManagedVolume<A>,
    expected_cursor: ChangeCursor,
    commit: &NamespaceCommit,
