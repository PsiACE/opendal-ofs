...
use crate::Error;
use crate::data::{VolumeAccess, validate_file_map};
use crate::filesystem::{
    ChangeCursor, NamespaceNode, NamespaceRecord, NamespaceValue, NodeId, VolumeId,
    validate_portable_path,
};
use crate::format::{
    FileExtentMap, GcEpoch, NamespaceChangeSegment, NamespaceCommit, ObjectClass, StreamKind,
    StreamRef,
};
use crate::storage::{RecordStreamReader, RecordStreamSizer, RecordStreamWriter};
use crate::work::{
...
pub(super) async fn write_genesis<A: VolumeAccess>(
...
pub(super) async fn write_snapshot<A: VolumeAccess>(
    volume: &ManagedVolume<A>,
...
pub(super) async fn write_delta<A: VolumeAccess>(
    volume: &ManagedVolume<A>,
    delta: PlannedDelta,
...
impl ChangeSource {
    ...
    async fn remote<A: VolumeAccess>(
        volume: &ManagedVolume<A>,
        reference: StreamRef,
    ) -> Result<Self, Error> {
        Ok(Self {
...
pub(super) async fn read_views<A: VolumeAccess>(
    volume: &ManagedVolume<A>,
    workspace: &WorkContext,
...
fn read_from_downloads<A: VolumeAccess>(
    volume: &ManagedVolume<A>,
...
async fn download<A: VolumeAccess>(
    volume: &ManagedVolume<A>,
    workspace: &WorkContext,
    reference: StreamRef,
    cursor: ChangeCursor,
...
fn validate_record<A: VolumeAccess>(
    volume: &ManagedVolume<A>,
