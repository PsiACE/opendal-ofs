...
pub(super) fn reconcile(fixture: &Fixture, ofs: Ofs) {
    ...
    let data_segments =
        "local/managed-sync/reconcile/managed/0/objects/00000000000000000000/04-data-segment";
    let data_objects = fixture.storage_usage(data_segments).0;
    require_success(
        ofs.sync(&replica_a, &state_a, &storage),
        "install disjoint merge in A",
    );
    assert_eq!(
        fixture.storage_usage(data_segments).0,
        data_objects,
        "installing a merged remote generation does not publish unreachable data"
    );
    assert_eq!(
        tree_summary(&replica_a),
        tree_summary(&replica_b),
