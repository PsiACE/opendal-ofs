...
use super::cli::{ManagedStatus, output_text, volume_name};
use super::fixture::{CaseRoot, Fixture, container_http_proxy, tree_summary};
...
pub(crate) fn run(keep: bool) {
    ...
    let fixture = Fixture::new(keep, "bub", Default::default()).start_bub();
    fixture.create_bucket();
    let sessions = fixture.storage_url("bub/sessions");
    let skills = fixture.storage_url("bub/skills");
    ...
    assert_eq!(
        tree_summary(&evidence.path.join("sessions-a")),
        tree_summary(&evidence.path.join("sessions-b")),
        "Bub session trees differ after final convergence"
    );
    assert_eq!(
        tree_summary(&evidence.path.join("skills-a")),
        tree_summary(&evidence.path.join("skills-b")),
...
fn container_sync(
    ...
    let name = volume_name(storage);
    let mut arguments = vec!["ofs", "sync", name.as_str(), replica, "--state", state];
    for path in resolve {
